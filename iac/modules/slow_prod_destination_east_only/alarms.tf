...
resource "aws_cloudwatch_metric_alarm" "high_agg_kds_putrecs_failedrecs" {
  depends_on          = [module.kds-agg.kinesis_data_stream_name]
  ...
  tags                = var.tags
  dimensions = {
    StreamName = module.kds-agg.kinesis_data_stream_name
  }
...
resource "aws_cloudwatch_metric_alarm" "timestream_system_err" {
  ...
  tags                = var.tags
  dimensions = {
    DatabaseName = aws_timestreamwrite_database.timestream_db.id
...
resource "aws_cloudwatch_metric_alarm" "timestream_tbl_suc_req_latency" {
  ...
  dimensions = {
    DatabaseName = aws_timestreamwrite_database.timestream_db.id
    TableName    = aws_timestreamwrite_table.sessions_table.table_name
    Operation    = "WriteRecords"
  }
...
resource "aws_cloudwatch_metric_alarm" "lambda_sqs_agg_dlq_count_high" {
  ...
  tags                = var.tags
  dimensions = {
    QueueName = aws_sqs_queue.buf-sess-dlq.id
  }
  actions_enabled           = "true"
  insufficient_data_actions = []
  alarm_actions             = [aws_sns_topic.alert_topic.arn]
...
resource "aws_cloudwatch_metric_alarm" "kda_not_pushing_to_aggregate_kds" {
  ...
  alarm_description   = "Flink has not sent any records to KDS in the last 5 minutes.  Please verify the Flink Agggregator is running."
  comparison_operator = "LessThanOrEqualToThreshold"
  evaluation_periods  = "1"

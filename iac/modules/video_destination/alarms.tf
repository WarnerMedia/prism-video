...
resource "aws_cloudwatch_metric_alarm" "too_many_requests" {
  depends_on          = [module.video_destination_detail.lb_arn_suffix]
  ...
  tags                = var.tags
  dimensions = {
    LoadBalancer = module.video_destination_detail.lb_arn_suffix
  }
...
resource "aws_cloudwatch_metric_alarm" "high_running_task_count" {
  depends_on          = [module.video_destination_detail.ecs_cluster_name, module.video_destination_detail.ecs_service_name]
  ...
  tags                = var.tags
  dimensions = {
    ClusterName = module.video_destination_detail.ecs_cluster_name
    ServiceName = module.video_destination_detail.ecs_service_name
  }
...
resource "aws_cloudwatch_metric_alarm" "high_raw_kds_putrecs_failedrecs" {
  depends_on          = [module.video_destination_detail.kinesis_stream_name_raw]
  ...
  tags                = var.tags
  dimensions = {
    StreamName = module.video_destination_detail.kinesis_stream_name_raw
  }
...
resource "aws_cloudwatch_metric_alarm" "high_agg_kds_putrecs_failedrecs" {
  depends_on          = [module.video_destination_detail.kinesis_stream_name_sess]
  ...
  tags                = var.tags
  dimensions = {
    StreamName = module.video_destination_detail.kinesis_stream_name_sess
  }
...
resource "aws_cloudwatch_metric_alarm" "kds_raw_dlq_count_high" {
  ...
  tags                = var.tags
  dimensions = {
    QueueName = module.video_destination_detail.sqs_raw_dlq_id
  }
  actions_enabled           = "true"
  insufficient_data_actions = []
  alarm_actions             = [aws_sns_topic.alert_topic.arn]
...
resource "aws_cloudwatch_metric_alarm" "kda_downtime_agg" {
  depends_on          = [module.video_destination_detail.kinesis_kda_name_agg]
  ...
  tags                = var.tags
  dimensions = {
    Application = module.video_destination_detail.kinesis_kda_name_agg
  }
...
resource "aws_cloudwatch_metric_alarm" "timestream_system_err" {
  depends_on          = [module.video_destination_detail.timestream_db_name]
  ...
  tags                = var.tags
  dimensions = {
    DatabaseName = module.video_destination_detail.timestream_db_name
...
resource "aws_cloudwatch_metric_alarm" "timestream_tbl_suc_req_latency" {
  depends_on          = [module.video_destination_detail.timestream_db_name, module.video_destination_detail.timestream_session_name]
  ...
  dimensions = {
    DatabaseName = module.video_destination_detail.timestream_db_name
    TableName    = module.video_destination_detail.sessions_table_name
    Operation    = "WriteRecords"
  }
...
// late arriving alert
resource "aws_cloudwatch_metric_alarm" "kda_agg_late_arriving" {
  alarm_name          = "${local.ns}-kda-agg-late-arriving"
  alarm_description   = "Late Arriving Count for Aggregate Flink App is higher than expected"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  datapoints_to_alarm = "1"
  metric_name         = "${local.ns}-late-record-count"
...
//SNS SQS Error Count
resource "aws_cloudwatch_metric_alarm" "dmt_s3_sqs_lambda_error" {
  alarm_name          = "${local.ns}-dmt-lambda-error"
  alarm_description   = "Error count for SNS/SQS lambda has exceeded threshold"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  datapoints_to_alarm = "1"
  metric_name         = "${local.ns}-SNS-SQS-error-count"
  namespace           = "lambda-custom-metrics"
  period              = "60"
  statistic           = "Sum"
  threshold           = "10"
...
resource "aws_cloudwatch_metric_alarm" "lambda_sqs_agg_dlq_count_high" {
  ...
  tags                = var.tags
  dimensions = {
    QueueName = module.video_destination_detail.sqs_lambda_dlq_buf_sess_id
  }
  actions_enabled           = "true"
  insufficient_data_actions = []
  alarm_actions             = [aws_sns_topic.alert_topic.arn]

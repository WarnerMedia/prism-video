resource "aws_sqs_queue" "buf-sess-dlq" {
  name                      = "${local.ns}-buf-sess-dlq"
  delay_seconds             = 0
  ...
  tags                              = var.tags
  kms_master_key_id                 = aws_kms_key.buf-sess-sqs.arn
  kms_data_key_reuse_period_seconds = 86400
}

resource "aws_kms_key" "buf-sess-sqs" {
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Principal": {
        "AWS": [
          "${aws_iam_role.iam_lambda_buf_sess.arn}"
        ]
      },
      "Action": [
        "kms:Encrypt",
